...
// 1. Interface para aceitar o preço vindo do servidor
interface ProductShowcaseProps {
  price: number;
}

export default function ProductShowcase({ price }: ProductShowcaseProps) {
  const { openModal } = useModal();

  // Lógica para separar os centavos (Ex: 99.90 vira integers: 99 e cents: 90)
  const integerPart = Math.floor(price);
  const centsPart = (price % 1).toFixed(2).substring(2);
        ...
        <div className="absolute bottom-6 md:bottom-10 left-1/2 -translate-x-1/2 w-full max-w-md px-4 z-30">
          <div className="bg-card/95 backdrop-blur-xl border border-sky-500/30 rounded-2xl p-6 text-center shadow-[0_0_50px_rgba(14,165,233,0.15)] animate-fade-in-up">
            ...
            <div className="inline-block w-full p-1 bg-gradient-to-r from-sky-500 to-blue-700 rounded-full shadow-[0_0_20px_rgba(14,165,233,0.3)]">
              <button
                onClick={openModal}
                className="bg-background text-foreground w-full py-4 rounded-full font-bold text-base md:text-lg tracking-widest uppercase hover:opacity-90 transition-colors flex items-center justify-center gap-2 cursor-pointer"
              >
