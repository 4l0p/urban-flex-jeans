export default function Features() {
  ...
  return (
    <section
      id="tecnologia"
      className="w-full bg-zinc-950 py-24 px-6 relative overflow-hidden"
    >
      {/* Luz de fundo (Efeito Glow Azul) */}
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {features.map((item, index) => (
            <div
              key={index}
              className="group p-8 rounded-2xl bg-zinc-900/50 border border-zinc-800 hover:border-sky-500/50 transition-all duration-500 hover:shadow-[0_0_30px_rgba(56,189,248,0.1)] hover:-translate-y-2"
            >
              <div className="w-14 h-14 bg-zinc-800 rounded-xl flex items-center justify-center mb-6 group-hover:bg-sky-500/10 transition-colors">
                {item.icon}
              </div>
